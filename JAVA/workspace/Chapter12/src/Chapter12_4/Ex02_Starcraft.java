...
interface Fly {
	void flyBuilding();
//	디폴드 메서드 : 인터페이스에 일반 메서드를 구현하는 기능
//	인터페이스에는 기본적으로 추상메서드만 사용 가능했으나 메서드가 추가되었을때 수정이 힘듬
//	추상 메서드를 추가하면 기존에 인터페이스를 사용하는 모든 클래스에서 에러가 발생함
//	추상 메서드는 반드시 구현해야하지만 동일한 내용이라면 추가하는 것이 굉장히 비효율적
...
class Barracks extends Building implements Fly {
	...
	public void flyBuilding() {
		System.out.println("건물이 제트엔진으로 날"
				+ "아서 이동하게 합니다.");
	}
}
