...
//		indexOf("찾고싶은 문자열") : 문자열의 위치를 찾아서 돌려주는 메서드
		...
		System.out.println(str.indexOf("a")); // 6 : 6번쨰
		System.out.println(str.indexOf("Banana")); // 5 : 5번째
//		indexOf("문자열", 시작위치) : 시작위치부터 문자열을 찾아서 돌려주는 메서드
		...
		} else {System.out.println("문자열에 바나나가 없습니다.");}
//		substring(시작위치, 종료위치) : 시작위치부터 종료위치까지의 문자열을 돌려주는 메서드
		System.out.println(str.substring(0,5)); // Apple
//		substring(시작위치) : 시작위치부터 문자열의 끝까지 돌려주는 메서드
		System.out.println(str.substring(11)); // Orange
...
//		String.valueOf(값) : 다른 타입의 자료형을 String으로 바꿔주는 메서드
		double e = 2.718281;
