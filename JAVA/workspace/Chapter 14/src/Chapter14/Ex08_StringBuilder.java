...
//		StringBuilder는 위의 문제점을 해결하여 연산속도가 빠름
		...
		buf.append(12345);
		System.out.println(buf.toString());
//		delete(0, 4) : 0번 위치에서 4번 위치까지 삭제하는 메서드
		buf.delete(0, 4);
		System.out.println(buf.toString());
//		replace(시작위치, 종료위치, 변경할 문자열) : 시작위치부터 종료위치-1까지의 문자를 변경할 문자열로 치환 
		buf.replace(4, 8, "ABC");
		System.out.println(buf.toString());
...
//		1970년 1월1일부터 현재까지의 밀리세컨드 계산한 값
		...
		for(int i=0; i<10000; i++) {
			str2.append("apple"); // str변수에 apple을 천개 저장
		}
