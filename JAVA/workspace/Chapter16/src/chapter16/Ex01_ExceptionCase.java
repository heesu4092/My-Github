...
//		컴파일 에러 : 컴파일 할떄 일어나는 문제, 이클립스(IDE)
//		in a = 0; 빨간색 줄로 알려주는 에러들 -> 코드 자체를 수정하여 해결해야 함
//		런타임 에러 : 실행도중 발생하는 에러, 값을 잘못 설정하거나 0으로 나눈다거나 하는 에러
//		런타임 에러의 종류 : 시스템 에러, 예외
//			시스템 에러 : 메모리 부족, 운영체제에서 에러가 발생
//		    예외(Exception) : 값을 잘못 설정하거나 0으로 나눈다거나 하는 에러
//			예외처리 : try catch문을 이용하여 예외가 발생하더라도 프로그램이 꺼지지 않고 계속 진행되도록 코드를 작성 
		Scanner sc = new Scanner(System.in);
//		try : 예외가 발생할 가능성이 있는 코드를 작성
		try {
			int num1 = sc.nextInt(); // java.util.InputMismatchException : 문자열을 입력하면 예외 발생
			int num2 = 10 / num1; // java.lang.ArithmeticException: / by zero : num1에 0을 입력하면 예외 발생
...
//			정상실행이던 예외가 일어나던 상관없이 무조건 실행되는 코드
			System.out.println("finally 부분 : 무조건 실행되는 코드");
		}
//		예외처리시 프로그램이 계속 실행됨
		System.out.println("프로그램이 정상적으로 종료 했습니다.");
